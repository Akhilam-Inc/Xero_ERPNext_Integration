...
def sync_voided_invoices():
	...
	try:
		...
		frappe.log_error(f"Found {len(voided_invoices)} voided invoices in Xero for today", "Voided Invoice Sync")
...
def process_voided_invoice(xero_invoice):
	...
	try:
		...
		if not sales_invoices:
			frappe.log_error(f"No ERPNext invoice found for Xero invoice {invoice_id} ({invoice_number})", "Voided Invoice Sync")
			return
		...
		if sales_invoice["docstatus"] != 1:
			frappe.log_error(f"Invoice {sales_invoice['name']} is not submitted, cannot cancel", "Voided Invoice Sync")
			return
		...
		frappe.log_error(f"Error processing voided invoice {xero_invoice.get('InvoiceID', 'Unknown')}: {str(e)}", "Voided Invoice Sync")
...
def cancel_invoice_in_erpnext(sales_invoice, xero_invoice_id, xero_invoice_number):
	...
	try:
		...
		frappe.log_error(f"Successfully cancelled invoice {sales_invoice['name']} due to VOID in Xero", "Voided Invoice Sync Success")
